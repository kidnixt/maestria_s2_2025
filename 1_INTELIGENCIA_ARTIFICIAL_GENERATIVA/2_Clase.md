...
Sean X,YX, Y variables aleatorias:
...
- Sea XX = “tirar un dado” y YY = “lanzar una moneda”.  
    Como son procesos independientes:  
    p(X=3,Y=H)=p(X=3) p(Y=H)=16⋅12p(X=3, Y=H) = p(X=3)\,p(Y=H) = \frac{1}{6}\cdot\frac{1}{2}.
...
Sean X,Y,ZX, Y, Z variables aleatorias:

- **Regla del producto:**
    
    p(x,y,z)=p(x) p(y∣x) p(z∣x,y)p(x,y,z) = p(x)\,p(y|x)\,p(z|x,y)
- **Independencia condicional:**
    
    X⊥Z∣Y⟺p(x,z∣y)=p(x∣y) p(z∣y)X \perp Z \mid Y \quad \Longleftrightarrow \quad p(x,z|y) = p(x|y)\,p(z|y)
- De esto se deduce:
    
    X⊥Z∣Y    ⟹    p(x,y,z)=p(x) p(y∣x) p(z∣y)X \perp Z \mid Y \;\;\Longrightarrow\;\; p(x,y,z) = p(x)\,p(y|x)\,p(z|y)

**Ejemplo intuitivo:**

- XX = “nublado”, YY = “llueve”, ZZ = “llevo paraguas”.
    
- Dado Y=llueveY=\text{llueve}, el hecho de que esté nublado o no ya no afecta a si llevo paraguas → independencia condicional.
    

---

## 3. 🧮 Generalización a nn variables

Para X1,…,XnX_1, \dots, X_n:

- **Regla general del producto:**
    
    p(x1,…,xn)=∏i=1np(xi∣x1,…,xi−1)p(x_1,\dots,x_n) = \prod_{i=1}^n p(x_i \mid x_1,\dots,x_{i-1})
- Si cada variable depende **solo de la inmediata anterior**:
    
    p(x1,…,xn)=∏i=1np(xi∣xi−1)p(x_1,\dots,x_n) = \prod_{i=1}^n p(x_i \mid x_{i-1})
...
- Cada nodo = una variable aleatoria XiX_i.
    
- Un arco i→ji \to j indica que XiX_i influye en XjX_j.
    
- Cada nodo tiene una **distribución condicional** asociada:
    
    p(xi∣{xj:j→i})p(x_i \mid \{x_j : j \to i\})

La distribución conjunta se factoriza como:

p(x1,…,xn)=∏i=1np(xi∣padres(Xi))p(x_1,\dots,x_n) = \prod_{i=1}^n p(x_i \mid \text{padres}(X_i))

**💡 Ventaja:** Representa dependencias locales → evita calcular la distribución conjunta completa (exponencial en tamaño).

---

## 5. 📞 Ejemplo clásico: _Sally’s Alarm_

Variables:

- BB: Ladrón (Burglar)
    
- EE: Terremoto (Earthquake)
    
- AA: Alarma (Alarm)
    
- RR: Radio reporta terremoto
...
- Probabilidad de que John llame depende de la alarma:
    ...
    - p(J=1∣A=0)=0.05p(J=1|A=0)=0.05, p(J=1∣A=1)=0.90p(J=1|A=1)=0.90.
        
- Probabilidad de que Mary llame depende de la alarma:
    
    - p(M=1∣A=0)=0.01p(M=1|A=0)=0.01, p(M=1∣A=1)=0.70p(M=1|A=1)=0.70.
...
- Barber, D. _Bayesian Reasoning and Machine Learning_. Cambridge University Press, 2012 (Cap. 3).
...
👉 ¿Querés que además de este resumen te arme un **ejemplo en Python (con `pgmpy`)** para construir la red de la alarma y hacer inferencias (ej. calcular la probabilidad de robo dado que John llama)?